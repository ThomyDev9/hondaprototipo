...
SELECT
    ...
    COALESCE(
        NULLIF(TRIM(src.actividad_economica), ''),
        NULLIF(TRIM(SUBSTRING_INDEX(src.actividad_economica_tiempo, '|', 1)), '')
    ),
    COALESCE(
        NULLIF(TRIM(src.tiempo_actividad_economica_anios), ''),
        NULLIF(TRIM(SUBSTRING_INDEX(src.actividad_economica_tiempo, '|', -1)), '')
    ),
    NULLIF(TRIM(src.monto_solicitado), ''),
    ...
    COALESCE(NULLIF(TRIM(src.fuente_ingreso), ''), NULLIF(TRIM(src.tipo_relacion_laboral), '')),
